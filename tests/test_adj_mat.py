...
new = importlib.import_module("adjacency_matrices")      # note the spelling in repo
...
def test_spatial_adjacency():
    ...
    _assert_same(
        old.get_spatial_adjacency_matrix(SIGMA, N),
        new.get_spatial_adjacency_matrix(SIGMA, N),
    )
...
def test_laplacians(lap_fn):
    ...
    if lap_fn == "spatial":
        A_old = old.get_spatial_adjacency_matrix(SIGMA, N)
        A_new = new.get_spatial_adjacency_matrix(SIGMA, N)
    else:
...
def test_get_laplacians():
    _banner("Testing combined get_laplacians()")
    Ls_old = old.get_laplacians(SIGMA)
    Ls_new = new.get_laplacians(SIGMA)
