...
from algonauts.models.graph_models import pad_to_fixed_length, FixedNetworkGNN,FixedNetworkGraphAttention1D,FixedNetworkGraphAttention
...
class PredictionTransformerRoPE_GAT(nn.Module):
    def __init__(self,
                 input_dim=256,
                 output_dim=1000,
                 num_layers=1,
        ...
        - adjacency_matrix: adjacency matrix for graph convolutions 
        """
        super().__init__()
        ...
        self.graph_layers = nn.ModuleList([
            FixedNetworkGraphAttention1D(adjacency_matrix=adjacency_matrix) for _ in range(num_layers)
        ])

        self.gelu = nn.GELU()
      #  self.enc =  nn.ModuleList([nn.Linear(input_dim,output_dim)for _ in range(num_layers)
       # ])

       # self.dec = nn.ModuleList([nn.Linear(output_dim,input_dim) for _ in range(num_layers)
        #])
        self.enc= nn.Linear(input_dim,output_dim)
        self.dec= nn.Linear(output_dim,input_dim)
    ...
    def forward(self, x, attn_mask):
        ...
        for i in range(len(self.attention_layers)):
            ...
            x = self.attention_layers[i](x, attn_mask=causal, key_padding_mask=key_padding_mask)

            #x = x+self.gelu(self.dec[i](self.graph_layers[i](self.gelu(self.enc[i](x)))))
            x = x+self.gelu(self.dec(self.graph_layers[i](self.gelu(self.enc(x)))))
